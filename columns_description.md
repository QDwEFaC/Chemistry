...
## Примечания для моделирования

1. **Целевые:** `IC50, mM`, `CC50, mM`, `SI` — экспериментальные показатели эффективности и безопасности; остальные столбцы — **структурные предикторы**, вычисленные из 2D-структуры (SMILES) без участия в измерении IC50/CC50.
2. **Связь целевых:** SI = CC50 / IC50 (при согласованных единицах); при построении моделей SI можно предсказывать отдельно или выводить из предсказаний CC50 и IC50.
3. **Масштаб:** IC50 и CC50 в **миллимолях**; для регрессии часто применяют логарифмирование (pIC50, log CC50).
4. **Корреляции признаков:** многие `fr_*`, VSA и Chi/Kappa дескрипторы **сильно коррелируют**; при отборе признаков и интерпретации моделей это стоит учитывать.
5. **Источник дескрипторов:** имена и расчёты соответствуют модулю **RDKit** (`Descriptors`, `Fragments`, `BCUT2D`, `MolSurf` и др.).
